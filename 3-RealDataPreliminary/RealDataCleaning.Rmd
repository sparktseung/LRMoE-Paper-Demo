...
hist(freMTPLfreq$DriverAge[freMTPLfreq$DriverAge>90],
     main = "Histogram of DriverAge>60", xlab = "DriverAge")
```
...
Based on the previous analysis, we only need to delete invalid data or outliers based on `Exposure`, `CarAge` and `DriverAge`. In total, there are 1324 data points deleted. Data cleanng based for `ClaimAmount` is deferred to the next section.
