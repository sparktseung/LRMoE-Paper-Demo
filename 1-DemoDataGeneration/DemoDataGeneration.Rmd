...
This document contains the data generation process for the dataset `DemoData` included in the `LRMoE` package. This also serves as an example of using the `dataset.simulator` function included in the package.
...
```{r, eval=TRUE}
Y.complete = LRMoE::dataset.simulator(X, alpha, comp.dist, zero.prob, params.list)
```
...
save(X, Y, X.obs, Y.obs, file = "DemoData.RData")
