...
We can also obtain the QQ plot of the fitted models against the true model. It is in theory possible to calculate the exact cumulative distribution function (CDF) of the fitted distribution at selected points, but it may be quite computationally intensive when the sample size is large, as it involves averaging the individually fitted distributions over all observations. 

For simplicity, we simulate from the fitted distribution and make the plots. The error may be controlled by choosing a relatively large sample size of simulation.
